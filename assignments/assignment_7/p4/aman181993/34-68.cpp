...
int f (int index, int column, int m1, int m2, int flag)
{
    ...
    if (index != n-1 && column != 1 && !check(m1,0) && !check(m1,1) && !check(m2,0)) {
        result = min (result, max (a[index][0], max (a[index][1], a[index+1][0])) + f (index, column+1, 3, m2 | (1), 0));
    }

    if (index != n-1 && column != 1 && !check(m1,0) && !check(m1,1) && !check(m2,1)) {
        result = min (result, max (a[index][0], max (a[index][1], a[index+1][1])) + f (index, column+1, 3, m2 | (2), 0));
    }

    if (index != n-1 && column != 1 && !check(m1,0) && !check(m2,1) && !check(m2,0)) {
        result = min (result, max (a[index][0], max (a[index+1][1], a[index+1][0])) + f (index, column+1, m1|(1), 3, 0));
    }
    ...
    if (m1 == 0 && m2 == 0 && index != n-1) {
        int xxx = max(a[index][0], a[index+1][0]) + max(a[index][1],a[index+1][1]);

        result = min (result, xxx+f(index+2, 0, 0, 0, flag+1));
    }
